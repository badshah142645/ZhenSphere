...
export default function ChatInput({ 
  ...
  const handleFiles = async (files: FileList) => {
    if (!projectId || preferredCli === 'cursor' || preferredCli === 'qwen' || preferredCli === 'grok') return;
  ...
  const handleDragEnter = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (projectId && preferredCli !== 'cursor' && preferredCli !== 'qwen' && preferredCli !== 'grok') {
      setIsDragOver(true);
    }
  };
  ...
  return (
    <div className="flex max-h-[calc(100%-37px)] shrink-0 flex-col overflow-visible">
      ...
      >
        ...
        {/* Drag overlay */}
        {isDragOver && projectId && preferredCli !== 'cursor' && preferredCli !== 'qwen' && preferredCli !== 'grok' && (
        ...
        <div className="flex items-center gap-1">
          <div className="flex items-center gap-2">
            {/* Image Upload Button */}
            {projectId && (
              (preferredCli === 'cursor' || preferredCli === 'qwen' || preferredCli === 'grok') ? (
                <div
                  ...
                  title={preferredCli === 'qwen' ? "Qwen Coder doesn't support image input" : preferredCli === 'grok' ? "Grok CLI doesn't support image input" : "Cursor CLI doesn't support image input"}
                >
                  ...
                  <>
                    ...
                    <span className="text-gray-500 dark:text-gray-500">
                      {selectedModel === 'claude-sonnet-4' ? 'Sonnet 4' :
                       ...
                       selectedModel === 'grok-4-latest' ? 'Grok 4 Latest' :
                       selectedModel === 'grok-3-latest' ? 'Grok 3 Latest' :
                       selectedModel === 'grok-3-fast' ? 'Grok 3 Fast' :
                       selectedModel}
                    </span>
                  </>
                )}
